...
pm_dat_tmp <- pm_dat %>% 
  ...
  mutate(item_pub_date = format(item_pub_date, format="%d.%m.%Y")) 
cat('<dl class="row"><div class = "col-sm-6">')
pm_dat_tmp %>% 
  slice(1:3) %>%
  mutate(html = glue('
  <dd class="col-sm-12  yellow-line">{item_pub_date} - {pub_type}</dd>
  <dd class="col-sm-12"><a class = "laake-link" href="{scraped_doi}"><i>{item_title}</i><i class="bi-chevron-right"></i></a></dd>
